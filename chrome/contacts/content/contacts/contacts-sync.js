...
cm2SynchroContact.prototype={
    ...
    if (null==this.oldCard){
      ...
      if (""==key){
        ...
        cm2DavTrace("Synchronisation d'un nouveau contact identifiant:"+key);
				//dump("*** synchronise nouveau contact key:"+key+"\n");
  ...
  updateServeur:function(card){
    ...
    cm2DavTrace("Synchronisation d'un contact - envoi du contact au serveur key:"+key);
		dump("updateServeur - envoi du contact au serveur key:"+key+"\n");
...
function cm2SynchroniseListe(newList, oldList=null){
	...
	//dump("*** Synchronisation d'une liste cm2SynchroniseListe\n");
  //return;//désactivé
  let synchroniseur;
...
cm2SynchroListe.prototype={
  ...
  synchronise:function(){
    ...
    if (null==this.oldList){
      ...
      cm2DavTrace("Synchronisation d'un nouvelle liste identifiant:"+key);
			dump("Synchronisation d'un nouvelle liste identifiant:'"+key+"'\n");
    ...
    cm2DavTrace("Modification d'une liste:"+key);
  ...
  onListUpload:function(status, response, headers, data){
    ...
    cm2DavTrace("Synchronisation d'une liste - reponse du serveur status:"+status);
		//dump("*** Synchronisation d'une liste - reponse du serveur status:"+status+" - response:"+response+"\n");
    if (status > 199 && status < 400) {
  ...
  updateServeur:function(liste){
    ...
    // liste
    this.envoieListe(liste);
  },
	
	envoieListe:function(liste){
										
		if (_this._nbUploadContacts!=0){
			//dump("*** envoieListe setTimeout\n");
			setTimeout(_this.envoieListe, 500, liste);
			
		} else{
			//dump("*** envoieListe\n");
			let key=_this.attrs.key;
			let card=_this.getCardForList(liste);
			let vList=list2vcard(key, card);
			//dump("*** envoieListe key'"+key+"' - vList:'"+vList+"'\n");
			let listUrl=_this.srvurl+key;
			let data={query: "list-upload",
								data: card,
								key: key};
			let request=new sogoWebDAV(listUrl, _this, data);
			_this.typereq=CM2_SYNCHRO_PUT_LIST;
			cm2DavTrace("envoieListe - envoi de la liste au serveur url:"+listUrl);
  ...
  mergeModifications:function(modifie, original, serveur){
    ...
    cm2DavTrace("Fusion des modifications serveur et client");
		//dump("*** Fusion des modifications serveur et client");
    let bmodif=false;
  ...
  envoieContacts:function(liste){
    ...
    for (let index=0; index<nbCards; index++) {
      ...
      dump(" envoieContacts contact key:'"+key+"'\n");
			//dump("*** kETagKey'"+card.getProperty(kETagKey, "")+"'\n");
      if (""==key){
  ...
  uploadContact:function(card){
    ...
    var retourCO={
      ...
      onDAVQueryComplete: function(status, response, headers, data) {
				...
				_this._nbUploadContacts--;
				//dump("*** uploadContact onDAVQueryComplete _nbUploadContacts:'"+_this._nbUploadContacts+"'\n");
        ...
        if (status > 199 && status < 400) {
          ...
          if (etag && etag.length) {
            ...
            //dump("*** onDAVQueryComplete nouveau etag serveur:"+etag+"\n");
          } else{
    ...
    if (""==key){
      ...
      this.abook.modifyCard(card);
		...
		//dump("*** uploadContact _nbUploadContacts:'"+this._nbUploadContacts+"'\n");
