...
function SCAbConfirmDelete(types) {
    ...
    if (types != kNothingSelected) {
        ...
        if (types == kListsAndCards)
            ...
            confirmDeleteMessage = gAddressBookBundle.getString("confirmDelete2orMoreMailingLists");
        else if (types == kCardsOnly && gAbView && gAbView.selection) {
             if (1==numSelectedItems){
                confirmDeleteMessage = gAddressBookBundle.getString("confirmDeleteThisContact");
...
function SCAbConfirmDeleteDirectory(selectedDir) {
    ...
    if (prefBranch.getCharPref("mail.collect_addressbook") == selectedDir
        ...
        confirmDeleteMessage = gAddressBookBundle.getString("confirmDeleteThisCollectionAddressbook");
    }
    else
    ...
    return (promptService.confirm(window,
                                  gAddressBookBundle.getString("confirmDeleteThisAddressbookTitle"),
...
function SCSetSearchCriteria(menuitem) {
    ...
    if (criteria.length > 0) {
        ...
        let prefBranch = (Components.classes["@mozilla.org/preferences-service;1"]
                .getService(Components.interfaces.nsIPrefBranch));
        let nameOrEMailSearch = prefBranch.getStringPref("mail.addr_book.quicksearchquery.format");
