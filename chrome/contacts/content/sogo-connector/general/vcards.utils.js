...
let _insertCardMethods = {
    ...
    email: function(props, parameters, values) {
        ...
        else if (types.indexOf("EMAIL3") > -1) {
            props["ThirdEmail"] = values[0];
        }
        else {
...
function card2vcard(card) {
    ...
    //bug mantis 0002625: Passer à 3 le nombre d'adresse mail gérées pas le Courrielleur
    //bug mantis 0002641: Les correspondances d'adresses courriel dans le sens courrielleur vers horde ne sont pas toujours correctes
    let primaryEmail = card.getProperty("PrimaryEmail", "");
    let secondEmail = card.getProperty("SecondEmail", "");
    let thirdEmail = card.getProperty("ThirdEmail", "");
...
function updateListFromVList(listCard, vListString, cards) {
    ...
    for (let i = 0; i < parsedString.length; i++) {
        ...
        if (line.tag == "fn") {
            ...
            if (!card && line.parameters["email"]) {
                let email = line.parameters["email"][0];
                if (email) {
                  email=email.replace(/[\r\n\s]*/g, "");
                  dump("_findCardWithEmail email:'"+email+"'\n");
                  listUpdated = true;
                  card = _findCardWithEmail(cards, email);
                }
                else dump("_findCardWithEmail pas d'email\n");
            }
            if (card)
